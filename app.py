...
# Lista total de ativos (dividiremos em dois grupos)
TODOS_ATIVOS = [
...
def analisar_com_tradingview(ativo):
    try:
        handler = TA_Handler(
            symbol=ativo,
            screener="crypto" if ativo.startswith(("BTC", "ETH", "XRP", "LTC", "BCH", "DOGE", "SOL")) else "forex",
            exchange="BINANCE" if ativo.startswith(("BTC", "ETH", "XRP", "LTC", "BCH", "DOGE", "SOL")) else "FX_IDC",
            interval=INTERVALO
        )
...
def analisar_com_yfinance(ativo):
    try:
        time.sleep(2)  # Delay extra para evitar bloqueios do Yahoo
...
def main():
    if not TOKEN or not CHAT_ID:
        ...
        return
    logging.info("Bot iniciado com múltiplas fontes e controle de grupos.")
    while True:
        ativos = next(grupos)
        for ativo in ativos:
        ...
        logging.info(f"Ciclo concluído com {len(ativos)} ativos. Aguardando {DELAY_CICLO}s...")
